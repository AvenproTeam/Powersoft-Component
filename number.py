    ...
    if isinstance(channels, dict):
        ...
        for channel in channel_list:
            ...
            if channel_num is not None:
                entities.extend([
                    PowersoftGainNumber(coordinator, api, channel_num),
                    PowersoftDelayNumber(coordinator, api, channel_num),
                ])
...
class PowersoftGainNumber(PowersoftNumberBase):
    ...
    def __init__(self, coordinator, api, channel: int) -> None:
        ...
        super().__init__(coordinator, api, channel)
        self._attr_unique_id = f"{coordinator.config_entry.entry_id}_ch{channel}_gain"
    ...
    async def async_set_native_value(self, value: float) -> None:
        """Set the gain value."""
        await self._api.set_gain(self._channel, value)
...
class PowersoftDelayNumber(PowersoftNumberBase):
    ...
    def __init__(self, coordinator, api, channel: int) -> None:
        ...
        super().__init__(coordinator, api, channel)
        self._attr_unique_id = f"{coordinator.config_entry.entry_id}_ch{channel}_delay"
    ...
    async def async_set_native_value(self, value: float) -> None:
        """Set the delay value."""
        await self._api.set_delay(self._channel, value)
        await self.coordinator.async_request_refresh()
